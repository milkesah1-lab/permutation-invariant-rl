...
from PI_network import PIselfAttentionFeedForwardNN
...
class PPO:
	...
	def __init__(self, env, **hyperparameters):
		...
		self.actor = PIselfAttentionFeedForwardNN(out_dim=self.act_dim,num_of_features=num_of_features,num_of_vehicles=num_of_vehicles)     # ALG STEP 1
		self.critic = PIselfAttentionFeedForwardNN(out_dim=1,num_of_features=num_of_features,num_of_vehicles=num_of_vehicles)
		self.actor.to(self.device)
		self.critic.to(self.device)

		# Initialize the fixed diagonal covariance matrix used by the actor.
	...
	def set_env(self, env):
		"""
			...
			This is useful for curriculum learning where each stage shares the
	...
	def _log_summary(self):
		...
		avg_ep_rews = float(np.mean([np.sum(ep_rews) for ep_rews in self.logger['batch_rews']]))
		avg_actor_loss = float(np.mean([losses.float().mean() for losses in self.logger['actor_losses']]))

		# Round decimal places for more aesthetic logging messages
		avg_ep_lens_rounded = str(round(avg_ep_lens, 2))
		avg_ep_rews_rounded = str(round(avg_ep_rews, 2))
		avg_actor_loss_rounded = str(round(avg_actor_loss, 5))
		# Print logging statements
		print(flush=True)
		print(f"-------------------- Iteration #{i_so_far} --------------------", flush=True)
		print(f"Average Episodic Length: {avg_ep_lens_rounded}", flush=True)
		print(f"Average Episodic Return: {avg_ep_rews_rounded}", flush=True)
		print(f"Average Loss: {avg_actor_loss_rounded}", flush=True)
		...
		self._append_csv_row(
			...
			avg_episodic_length=avg_ep_lens,
			avg_episodic_return=avg_ep_rews,
			avg_loss=avg_actor_loss,
