...
class ContinuousSpawnHighwayEnv(HighwayEnv):
    ...
    - traffic is spawned around the ego instead of far from it
    - reset scenarios can be tuned for curriculum learning
    - new traffic can continue to spawn during the episode
    """
    ...
    def reset(self, *args, **kwargs):
        ...
        if self.config.get("enforce_speed_each_step", False):
    ...
    def _build_scenario(self, vehicle_class, ego_lane_id: int) -> None:
        ...
        if self._active_scenario == "overtake_blocked":
            ...
            front_blocker = self._make_vehicle_near_ego(
                ...
                distance_range=tuple(self.config.get("blocked_adjacent_front_distance_range", [18.0, 30.0])),
                speed_range=tuple(self.config.get("blocked_adjacent_speed_range", [22.0, 27.0])),
                allow_lane_change=False,
                min_gap_override=float(self.config.get("blocked_adjacent_front_min_gap", 10.0)),
            )
            ...
            rear_blocker = self._make_vehicle_near_ego(
                ...
                ahead=False,
                forced_lane_id=blocked_lane_id,
                distance_range=tuple(self.config.get("blocked_adjacent_rear_distance_range", [15.0, 25.0])),
                speed_range=tuple(self.config.get("blocked_adjacent_speed_range", [22.0, 27.0])),
                allow_lane_change=False,
                min_gap_override=float(self.config.get("blocked_adjacent_rear_min_gap", 10.0)),
            )
            if rear_blocker is not None:
                self.road.vehicles.append(rear_blocker)

            for lane_id in semi_open_lane_ids:
                front_gap_vehicle = self._make_vehicle_near_ego(
                    vehicle_class,
                    ahead=True,
                    forced_lane_id=lane_id,
                    distance_range=tuple(self.config.get("semi_open_adjacent_front_distance_range", [32.0, 48.0])),
                    speed_range=tuple(self.config.get("semi_open_adjacent_speed_range", [22.0, 26.0])),
                    allow_lane_change=False,
                )
                if front_gap_vehicle is not None:
                    self.road.vehicles.append(front_gap_vehicle)
            return
        ...
        if self._active_scenario == "stay_best":
            lead_vehicle = self._make_vehicle_near_ego(
                ...
                distance_range=tuple(self.config.get("stay_best_lead_distance_range", [65.0, 90.0])),
                speed_range=tuple(self.config.get("stay_best_lead_speed_range", [24.0, 27.0])),
                allow_lane_change=False,
            )
            if lead_vehicle is not None:
                self.road.vehicles.append(lead_vehicle)
    ...
    def _spawn_same_lane_blocker(
        ...
        if distance_range is None:
            distance_range = tuple(self.config.get("same_lane_blocker_distance_range", [30.0, 44.0]))

        if speed_range is None:
            speed_range = tuple(self.config.get("same_lane_blocker_speed_range", [15.0, 19.0]))
    ...
    def _should_spawn_same_lane_blocker(self) -> bool:
        ...
        interval = int(self.config.get("same_lane_blocker_check_interval", 10))
        if interval <= 0 or self._spawn_step % interval != 0:
            return False

        lookahead = float(self.config.get("same_lane_blocker_lookahead", 50.0))
        if self._has_same_lane_leader(lookahead):
            return False

        probability = float(self.config.get("same_lane_blocker_probability", 0.25))
    ...
    def _has_same_lane_leader(self, max_distance: float) -> bool:
        ...
        min_speed_delta = float(self.config.get("close_follow_speed_diff", 1.0))
    ...
    def _close_follow_penalty(self) -> float:
        ...
        safe_distance = float(self.config.get("close_follow_distance", 24.0))
        min_speed_diff = float(self.config.get("close_follow_speed_diff", 1.0))
    ...
    def _slow_leader_penalty(self) -> float:
        ...
        ego_lane = int(self.vehicle.lane_index[2])
        ego_x = float(self.vehicle.position[0])
        max_distance = float(self.config.get("slow_leader_distance", 55.0))
        min_speed_diff = float(self.config.get("slow_leader_speed_diff", 1.5))
    ...
    def _make_vehicle_near_ego(
        ...
        for _ in range(20):
            ...
            if speed_range is None:
                speed = self.np_random.uniform(*self._speed_range())
            else:
    ...
    def _speed_range(self):
        speed_range = self.config.get("traffic_speed_range", [20.0, 27.0])
        if not isinstance(speed_range, (list, tuple)) or len(speed_range) != 2:
            speed_range = [20.0, 27.0]
