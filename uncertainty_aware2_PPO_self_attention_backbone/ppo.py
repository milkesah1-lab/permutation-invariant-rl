...
from PI_network import PIselfAttentionFeedForwardNN, PIDropoutCritic
...
class PPO:
	...
	def get_safeguarded_action(self, obs, threshold, mc_samples=10):
		...
		_ , V_var = self.estimate_critic_uncertainty(obs, mc_samples=mc_samples, return_samples=False, require_grad=False)
		uncertainty = V_var.mean().item()
		activated = uncertainty > threshold
		...
		action = torch.clamp(action, torch.as_tensor(self.env.action_space.low).to(self.device), torch.as_tensor(self.env.action_space.high).to(self.device))
		return action.detach().cpu().numpy(), uncertainty, activated 
