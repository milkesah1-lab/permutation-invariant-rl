...
def main():
    ...
    print(f"Network type: {os.environ.get('PPO_NETWORK_TYPE', 'mean_max')}")
    ...
    for stage_index, (config_name, stage_timesteps) in enumerate(stages, start=1):
        ...
        csv_log_path = csv_dir / f"{MODEL_LABEL}_{stage_index:02d}_{config_name}_training.csv"
        model.set_csv_log_path(csv_log_path)
        model.learn(total_timesteps=stage_timesteps)

        actor_path = checkpoints_dir / f"{MODEL_LABEL}_{stage_index:02d}_{config_name}_actor.pth"
        critic_path = checkpoints_dir / f"{MODEL_LABEL}_{stage_index:02d}_{config_name}_critic.pth"
