...
DEFAULT_RUN_ID = f"stage3_uncertainty_meanmax_threshold_3run_100k_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
...
def main() -> int:
    ...
    if len(TRAIN_SEEDS) != 3:
        raise ValueError(f"Expected 3 training seeds, got {len(TRAIN_SEEDS)}")
